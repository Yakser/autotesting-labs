...
class MainPageLocators:
    ...
    TIMETABLE_LINK = (
        By.XPATH,
        '//a[@href="https://rasp.dmami.ru/" and contains(@class, "btn") and contains(@class, "text-button")]',
    )
    GROUP_INPUT = (
        By.CSS_SELECTOR,
        ".header.not-print > div.header-search.search > input",
    )
...
class MainPage(BasePage):
    ...
    def go_to_timetable_section(self):
        time.sleep(1)
        self.wait.until(
            ec.element_to_be_clickable(MainPageLocators.GO_TO_TIMETABLE_BUTTON)
        ).click()

    @allure.step("Click timetable link")
    def click_timetable_link(self):
        self.wait.until(
            ec.element_to_be_clickable(MainPageLocators.TIMETABLE_LINK)
        ).click()

    def find_groups_by_number(self, group_number: str):
        self.wait.until(
            ec.element_to_be_clickable(MainPageLocators.GROUP_INPUT)
        ).send_keys(group_number)
        time.sleep(1)
        groups_ul = self.wait.until(
            ec.presence_of_element_located(MainPageLocators.GROUPS_LIST)
        )
        return groups_ul

    def get_today_schedule(self):
        return self.wait.until(
            ec.presence_of_element_located(MainPageLocators.TODAY_SCHEDULE)
        )
